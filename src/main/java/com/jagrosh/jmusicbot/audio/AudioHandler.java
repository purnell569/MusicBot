 ...
 *
 * Modified 2026: track-failure handling now detects YouTube sign-in failures across the
 * whole exception graph, including suppressed per-client errors and typographic
 * apostrophes.
 *
...
public class AudioHandler extends AudioEventAdapter implements AudioSendHandler 
{
    ...
    public void onTrackException(AudioPlayer player, AudioTrack track, FriendlyException exception) {
        ...
        if (requiresYoutubeLogin(exception))
        {
            LOGGER.error(
                    "Track {} has failed to play: {}. "
                            + "YouTube now requires a signed-in session for this video. "
                            + "Set playback.youtube.useOAuth = true in your config and restart, "
                            + "then follow the sign-in code printed here on startup. "
                            + "More info: https://jmusicbot.com/youtube-oauth2\n{}",
                    track.getIdentifier(),
                    describeYoutubeLoginFailure(exception),
    ...
    /**
     * Phrases YouTube returns when a track can only be played by a signed-in session.
     *
     * <p>Stored lowercase and with typographic apostrophes folded to ASCII, because
     * YouTube writes "you\u2019re" with U+2019 rather than an ASCII quote.
     */
    private static final String[] YOUTUBE_LOGIN_MARKERS = {
    ...
    /**
     * Reports whether a playback failure was YouTube demanding an authenticated session.
     *
     * <p>The check walks the entire throwable graph rather than testing
     * {@code exception.getMessage()} alone. youtube-source tries several clients and,
     * when they all fail, reports an {@code AllClientsFailedException} whose own message
     * is just "All clients failed to load the item." - the actionable per-client reasons
     * hang off it as causes and suppressed exceptions. Matching only the top-level
     * message meant this hint never fired for the failure it was written for.
     */
    private static boolean requiresYoutubeLogin(Throwable throwable)
    {
        return containsLoginMarker(flattenMessages(throwable));
    }

    /**
     * Returns the specific login-related reason from the throwable graph, for logging.
     */
    private static String describeYoutubeLoginFailure(Throwable throwable)
    {
        for (String message : flattenMessages(throwable).split("\n"))
        {
            if (containsLoginMarker(message))
            {
                return message.trim();
            }
        }
        return throwable.getMessage() != null ? throwable.getMessage() : "sign-in required";
    }

    private static boolean containsLoginMarker(String normalisedText)
    {
        for (String marker : YOUTUBE_LOGIN_MARKERS)
        {
            if (normalisedText.contains(marker))
            {
                return true;
            }
        }
    ...
    /**
     * Flattens the messages of a throwable, its causes and its suppressed exceptions into
     * one normalised, newline-separated block.
     */
    private static String flattenMessages(Throwable throwable)
    {
        return collectMessages(throwable, new HashSet<>(), new StringBuilder()).toString();
    }

    /**
     * Recursive worker for {@link #flattenMessages(Throwable)}. The visited set guards
     * against cycles, which self-referencing causes can otherwise turn into infinite
     * recursion.
     */
    private static StringBuilder collectMessages(Throwable throwable, Set<Throwable> visited, StringBuilder out)
    {
        ...
        if (message != null)
        {
            out.append(message.toLowerCase()
                    .replace('\u2019', '\'')
                    .replace('\u2018', '\''))
               .append('\n');
        }

        collectMessages(throwable.getCause(), visited, out);
        for (Throwable suppressed : throwable.getSuppressed())
        {
            collectMessages(suppressed, visited, out);
        }
        return out;
    }
