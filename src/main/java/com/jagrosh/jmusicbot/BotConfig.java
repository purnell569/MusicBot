 ...
 *
 * Modified 2026: added the YouTube remote cipher settings.
 *
...
public class BotConfig {
    ...
    private String youtubeRemoteCipherUrl, youtubeRemoteCipherPassword;
    ...
    private void loadConfigValues(Config config, Config migratedUserConfig) {
        ...
        youtubeRemoteCipherUrl = YOUTUBE_REMOTE_CIPHER_URL.hasValue(config)
                ? YOUTUBE_REMOTE_CIPHER_URL.getString(config) : "";
        youtubeRemoteCipherPassword = YOUTUBE_REMOTE_CIPHER_PASSWORD.hasValue(config)
                ? YOUTUBE_REMOTE_CIPHER_PASSWORD.getString(config) : "";
    ...
    /**
     * Gets the remote cipher server URL used to decipher YouTube stream signatures.
     *
     * <p>YouTube rotates its player script frequently, which repeatedly breaks the
     * signature extractor bundled with youtube-source ("Must find sig function from
     * script" failures). Offloading deciphering to a remote cipher server keeps
     * playback working without waiting for a bot update.
     *
     * @return the configured URL, or an empty string to use the local extractor
     */
    ...
    /**
     * Gets the password for the remote cipher server, if it requires one.
     *
     * @return the configured password, or an empty string if none is set
     */
