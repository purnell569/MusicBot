...
class AudioSourceOAuthTest {
    ...
    class OAuthConfigurationTests {
        ...
        void youtubeClientsConfiguredForOAuthMode() throws Exception {
            ...
            assertEquals(6, oauthClients.length, "OAuth mode should use 6 clients");
            ...
            assertEquals("AndroidWithThumbnail", oauthClients[5].getClass().getSimpleName(),
                "Sixth OAuth client should be AndroidWithThumbnail, the anonymous playback fallback");
        ...
        void youtubeClientsConfiguredForNonOAuthMode() throws Exception {
            ...
            // Order is load-bearing: the first client that can serve a request wins, so the
            // most reliable one leads and the SABR-prone WEB client trails.
            assertEquals("AndroidWithThumbnail", nonOauthClients[0].getClass().getSimpleName(),
                "ANDROID must lead - it is the only client YouTube still reliably streams to "
              + "anonymously; the others return 'requires login' / 'not a bot'");
            assertEquals("AndroidVrWithThumbnail", nonOauthClients[1].getClass().getSimpleName(),
                "Second non-OAuth client should be <AndroidVrWithThumbnail>");
            assertEquals("MWebWithThumbnail", nonOauthClients[2].getClass().getSimpleName(),
                "Third non-OAuth client should be <MWebWithThumbnail>");
            ...
            assertEquals("WebWithThumbnail", nonOauthClients[4].getClass().getSimpleName(),
                "WEB must trail - SABR-only responses make it the least likely to yield a format");
        }
