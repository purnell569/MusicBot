...
download() {
    if [ $DOWNLOAD = true ]; then
        ...
        LATEST_RELEASE=$(curl -s https://api.github.com/repos/purnell569/MusicBot/releases/latest)
        ...
        if [ "$IS_PRERELEASE" = "true" ]; then
            ...
            ALL_RELEASES=$(curl -s https://api.github.com/repos/purnell569/MusicBot/releases?per_page=10)
