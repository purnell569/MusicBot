...
> [!NOTE]
> **This is a fork of a fork.** Credit for the bot belongs to the two people below — this
> repository only fixes YouTube playback.
>
> | Author | Repository | Role |
> | --- | --- | --- |
> | **[jagrosh](https://github.com/jagrosh)** | [jagrosh/MusicBot](https://github.com/jagrosh/MusicBot) | Original author of JMusicBot |
> | **[arif-banai](https://github.com/arif-banai)** | [arif-banai/MusicBot](https://github.com/arif-banai/MusicBot) | Revived and maintains it — dependency updates, Java 25, config migration. **Direct parent of this fork.** |
>
> arif-banai's note on their own fork: *"This is a fork of JMusicBot from jagrosh. I started this
> because it was not maintained and was not working anymore."*
>
...
- **Signature cipher is no longer tied to OAuth.** Deciphering and authentication are unrelated
  ...
  to the bundled extractor and died with `Must find sig function from script: /s/player/…`.
  It is now always applied and configurable via `playback.youtube.remoteCipher.url`.
- **Added the ANDROID client and reordered the client list by reliability.** ANDROID is currently
  the only client YouTube reliably streams to anonymously; WEB now sits last because YouTube
  answers it with SABR-only responses that contain no playable format.
- **Sign-in failures are now reported usefully.** The existing hint could never fire: it matched
  on the aggregate exception message rather than the per-client reasons, and compared against an
  ASCII apostrophe while YouTube sends `’` (U+2019).
- **youtube-source pinned past 1.18.2**, which predates the fixes for the TV client User-Agent
  and for formats that omit `contentLength`.
